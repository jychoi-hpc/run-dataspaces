...
def getstds(args):
    if args.oe is None:
        f_stdout = open(args.stdout, 'w') if args.stdout is not None else subprocess.STDOUT
        f_stderr = open(args.stderr, 'w') if args.stdout is not None else subprocess.STDERR
    else:
        f = open(args.oe, 'w')
        f_stdout = f
        f_stderr = f
...
def main():
    ...
    for a in args_cmd_list:
        cl_cmd = ' '.join(['mpirun -n %d' % (a.np), ' '.join(a.UNKNOWN), ' '.join(a.CMDS)])
